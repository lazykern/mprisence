...
impl Config {
    ...
    pub fn effective_web_player_layer(&self, key: &str) -> WebPlayerConfigLayer {
        let mut effective = self
            .bundled_web_player
            .get("default")
            .map(WebPlayerConfigLayer::default_base)
            .unwrap_or_default();
        if key != "default" {
            if let Some(layer) = self.bundled_web_player.get(key) {
                effective.merge_from(layer.clone());
            }
        }
        if let Some(layer) = self.user_web_player.get("default") {
            effective.merge_from(layer.default_base());
        }
        if key != "default" {
            if let Some(layer) = self.user_web_player.get(key) {
                effective.merge_from(layer.clone());
...
mod web_player_tests {
    ...
    fn user_web_default_ignore_does_not_hide_matched_sites() {
        ...
        assert!(!matched.ignore, "default ignore must not hide a matched site");
    ...
    /// The layer feeding the config UI must agree with the runtime: a legacy
    /// `[web_player.default] ignore = true` must not render every site's
    /// switch as off while the daemon shows them on.
    #[test]
    fn effective_layer_ignores_legacy_default_ignore() {
        let mut cfg = Config::default();
        cfg.bundled_web_player.insert(
            "tidal".into(),
            WebPlayerConfigLayer {
        ...
        let effective = cfg.effective_web_player_layer("tidal");
        assert_eq!(effective.ignore, Some(false));
        let runtime =
            cfg.get_player_config_with_url("Firefox", "firefox", Some("https://tidal.com/track/1"));
        assert_eq!(effective.ignore, Some(runtime.ignore));
    }
    ...
    fn disabled_web_players_fall_through_to_player_rules() {
        ...
        let unknown =
            cfg.resolve_source("Firefox", "firefox", Some("https://unknown.example/x"), None);
